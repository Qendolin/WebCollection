export default class CanvasTransformer {
	...
	constructor(src) {
		...
		const promise = new Promise((resolve, reject) => {
			this._img.onload = () => {
				 ...
				 */
				this._canvas = []
				this._canvas[0] = document.createElement('canvas')
				this._canvas[0].width = this._img.width
				this._canvas[0].height = this._img.height
				this._ctx[0] = this._canvas[0].getContext('2d')
				this._ctx[0].drawImage(this._img, 0, 0)

				this._canvas[1] = document.createElement('canvas')
				this._canvas[1].width = this._img.width
				this._canvas[1].height = this._img.height
				this._ctx[1] = this._canvas[1].getContext('2d')
	...
	_swap() {
		const active = this._canvas[this._activeCanvas]
		const inactive = this._canvas[+!this._activeCanvas]

		const activeCtx = this._ctx[this._activeCanvas]
		const inactiveCtx = this._ctx[+!this._activeCanvas]
